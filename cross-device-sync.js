...
class CrossDeviceSync {
    ...
    async exportSyncData() {
        try {
            ...
            // 获取现有题目
            const publicQuestions = [...window.app.publicQuestionBank];
            const personalQuestions = [...window.app.personalQuestionBank];
            const mistakeBank = [...window.app.mistakeBank];
            ...
            try {
                ...
                reader.onload = async (e) => {
                    try {
                        ...
                        console.log('   - 公共题目:', importData.metadata?.publicQuestionCount || 0);
                        console.log('   - 个人题目:', importData.metadata?.personalQuestionCount || 0);
                        console.log('   - 错题记录:', importData.metadata?.mistakeCount || 0);
    ...
    mergeStudyStats(importedStats) {
        if (!window.app || !importedStats) return;
    ...
    showQRCodeDialog() {
        ...
        if (typeof QRCode === 'undefined') {
            this.showNotification('二维码功能暂不可用，请使用文件导入/导出功能', 'warning');
            return;
        }
        ...
        dialog.innerHTML = `
            <div class="bg-white rounded-lg p-6 max-w-md w-full">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900">扫码同步</h3>
                    <button onclick="this.closest('.fixed').remove()" class="text-gray-400 hover:text-gray-600">
                        <i class="fa fa-times"></i>
                    </button>
                </div>
                <div class="text-center mb-4">
                    <p class="text-sm text-gray-600 mb-4">请使用另一台设备扫描下方二维码进行同步</p>
        ...
        // 生成二维码
        try {
            const qrData = this.generateSyncQRData();
            ...
            this.showNotification('二维码生成失败，请重试', 'error');
            dialog.remove();
        }
    }
